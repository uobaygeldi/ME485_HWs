        ...
        def run(upts):
            ...
            for idx in range(neles):
                ...
                for i in range(nvars):
                    ...
                    # Regular Disk
                    '''r_i = (x[0] ** 2 + x[1] ** 2) ** 0.5
                    flux = - (muf*(T2-T1))/(r_i*np.log(r[1]/r[0])) # incropera c.5 (appendix) eqn is div of flux
                    eqn = -muf*(T2-T1)*(((x[0]-x[1])*(x[0]+x[1]))/(np.log(r[1]/r[0])*r_i)+((x[1]**2-x[0]**2))/(np.log(r[1]/r[0])*r_i))
